...
void SetEvidence(EvidenceByte* main, EvidenceByte toSet) {
    *main = *main | toSet;
}

void clearEvidence(EvidenceByte* main, EvidenceByte toClear) {
    *main = *main & ~toClear;

}

Room* pickConnectedRoom(Room* room) {
    int totalConnects = room->numOfConnectedRooms - 1;  //- 1 bc numOfConnectedRooms is 1 greater than array indexing
    int i = rand_int_threadsafe(0, totalConnects);
    Room* newRoom = room->connectedRooms[i];

    return newRoom;
}

bool pickRoomsAlphabetically(Room* room1, Room* room2) {
    if (strcmp(room1->roomName, room2->roomName) < 0) return true;
    return false;
}

void displayInvestigationResults(House* house) {
    ...
    for (int i = 0; i < house->hunterArray.size; ++i) {
        Hunter* h = &(house->hunterArray.hunters[i]);

        printf("  Hunter %-10s (id=%d): exit reason = %s, "
               "final boredom=%d, fear=%d\n",
               h->hunterName,
               h->hunterId,
               exit_reason_to_string(h->reasonForExiting),
               h->boredom,
               h->fear);
    }

    printf("\nCollected evidence in shared case file:\n");
    const enum EvidenceType* types = NULL;
    int type_count = get_all_evidence_types(&types);
    ...
    if (mask == 0) {
        ...
        for (int i = 0; i < type_count; ++i) {
            if (mask & types[i]) {
                printf("  - %s\n", evidence_to_string(types[i]));
            }
        }
    }
}
void displayVictoryResults(House* house) {
    printf("\nVictory Results:\n");
    printf("-------------------------\n");

    int totalHunters        = house->hunterArray.size;
    int evidenceExitCount   = 0;
    ...
    printf("- Hunters exited after identifying the ghost: %d/%d\n",
           evidenceExitCount, totalHunters);

    EvidenceByte mask = house->casefile.collected;
    const char* guessText = "N/A";

    if (evidence_has_three_unique(mask) && evidence_is_valid_ghost(mask)) {
        const enum GhostType* ghost_types = NULL;
        int ghost_count = get_all_ghost_types(&ghost_types);

        for (int i = 0; i < ghost_count; ++i) {
            if ((EvidenceByte)ghost_types[i] == mask) {
                guessText = ghost_to_string(ghost_types[i]);
                break;
            }
        }
    }
...
// ---- Thread-safe random number generation ---- NOTE: MADE UPPER INCLUSIVE.
int rand_int_threadsafe(int lower_inclusive, int upper_exclusive) {
    ...
    unsigned span = (unsigned)(upper_exclusive - lower_inclusive + 1u);
